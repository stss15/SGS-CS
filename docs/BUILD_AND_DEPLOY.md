...
The site uses a **Nunjucks templating system** that compiles `.njk` templates into static HTML.

```
src/pages/*.njk  →  npm run build  →  public/*.html
```
...
1. Scans `src/pages/` for `.njk` files
2. Reads frontmatter (YAML) and content
3. Applies layout template (e.g., `base.njk`, `slide-deck.njk`)
4. Computes `basePath` for relative links
5. Writes HTML to `public/` with `<!-- GENERATED FILE -->` marker
6. Updates `meta/site-manifest.json`
...
The CI workflow (`.github/workflows/pages.yml`) automatically:
...
# Apply keywords (optional, for IGCSE)
python3 public/igcse/apply_keywords.py
...
1. Run `python3 public/igcse/apply_keywords.py`
...
| `public/igcse/apply_keywords.py` | Keyword highlighting |
