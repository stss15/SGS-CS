...
| **Challenge Count** | 10 |
| **Prerequisites** | Levels 1-4 (especially Loops) |
...
1. Understand static vs dynamic arrays (Python lists are dynamic)
2. Create and initialize lists
3. Access elements by index (positive and negative)
4. Modify list elements
5. Use core list methods: `.append()`, `.pop()`, `.remove()`, `.insert()`
6. Traverse lists by element (`for item in list`)
7. Traverse lists by index (`for i in range(len(list))`)
8. Create and initialize 2D lists (matrices)
9. Access 2D list elements with `grid[row][col]`
10. Traverse 2D lists with nested loops
...
- **Level 1**: Variables (lists are variables holding multiple values)
- **Level 2**: Selection (checking list values)
- **Level 3**: Loops (essential for list traversal), accumulators/counters
- **Level 4**: String iteration pattern (same pattern works with lists)

### Key Integration:
- Lists + loops = data processing
- Lists + conditionals = filtering data
- Lists + accumulators = aggregating data
...
systems = [True, True, False, True]
coordinates = [45, 67, 89, 23]
...
### Section 5: Traversing by Element
...
1. **`data/level5_content.ts`** - Create with content above
2. **`data/roadmap.ts`** - Update Level 5 tasks

### Important IB Points (B2.2.1, B2.2.2)
- Static vs Dynamic: Mention Python lists are dynamic (can grow/shrink)
- Zero-based indexing is critical
- 2D arrays must use row, col convention
- Nested loops for 2D traversal

### Common Errors
- IndexError from out-of-range access
- Forgetting that pop() modifies the list
- Confusing element vs index traversal
- Wrong order in 2D: it's [row][col], not [col][row]
...
|-----------|------------|
| 1 | List, length 3, first/last names |
| 2 | Status transformations |
| 3 | Growing crew list |
| 4 | Shrinking crew list |
| 5 | Tasks in correct order |
| 6 | 3 names longer than 6 |
| 7 | First >80 at index 3 |
| 8 | Modified cargo grid |
| 9 | Specific values, 3 rows, 4 cols |
| 10 | All positions, total=201, count=5 |

---

*Level 5 Plan - Version 1.0*
