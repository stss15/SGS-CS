...
| **Syllabus** | B2.3.4 |
| **Challenge Count** | 8 |
| **Prerequisites** | Levels 1-5 (all foundational concepts) |
...
1. Define functions using `def`
2. Create functions with parameters (arguments)
3. Return values from functions using `return`
4. Return multiple values
5. Understand and apply local vs global scope
6. Break complex problems into smaller functions
7. Call functions from other functions
8. Document functions with meaningful comments

---

## Synoptic Connections

### This level SYNTHESIZES:
- **Level 1**: Variables, data types, I/O
- **Level 2**: Selection (used inside functions)
- **Level 3**: Loops (used inside functions)
- **Level 4**: Strings (passed as parameters)
- **Level 5**: Lists (passed and returned)

### Key Insight:
Functions let you package ALL previous concepts into reusable modules!
...
```
# Function names (action verbs)
calculate_fuel(), check_systems(), launch_sequence()
get_crew_status(), validate_coordinates(), process_telemetry()

# Parameters
fuel_amount, crew_count, target_distance
temperature, pressure, velocity

# Return values
status, result, is_valid, total
```
...
### Challenge 5: Multi-Return Analysis
...
1. **`data/level7_content.ts`** - Create with content above
2. **`data/roadmap.ts`** - Update Level 7 tasks:
```typescript
{
  id: 7,
  title: "Modular Programming",
...
### Key IB Points (B2.3.4)
- Subprograms (functions) for modularity
...
|-----------|------------|
| 1 | Status report printed twice |
| 2 | Three personalized greetings |
| 3 | 384.4 hours to Moon |
| 4 | GO, NO-GO |
| 5 | Min=78, Max=95, Avg=87.6 |
| 6 | Error when accessing local outside |
| 7 | SAFE, DANGER |
| 8 | Complete formatted report |

---

*Level 7 Plan - Version 1.0*
